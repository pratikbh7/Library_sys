...
spl_autoload_register( function($class){
    ...
    $class = PATH . '/' . ( empty($namespace) ? "" : $namespace . "/" ) . "{$classname}.php";
    include($class);
});
...
class HelperClass{
    ...
    public static function admin_user_exists( $link ){
    ...
    public static function register_admin_data( string $username, string $password, int $privileges, string $admin = "" ){
    ...
    public static function is_admin_logged_in():bool{
        ...
        if( $token && self::token_validity($token) ){
            $admin = self::find_admin_by_token($token);
            //populate session variables and log admin in
            if( $admin){
                return self::log_admin_in($admin);
            }
        }
        return false;
    }
    
    public static function log_admin_out(){
        if( is_user_logged_in()){
            self::delete_admin_token($_SESSION['userdata']['userid']);
    ...
    public static function log_admin_in( $admin ){
        if( !isset( $_SESSION['userdata']['username'] ) && !isset( $_SESSION['userdata']['username'] ) ){
            self::remember_admin($admin['userid']);
    ...
    public static function generate_tokens(): array{
    ...
    public static function insert_admin_token(int $admin_id, string $selector, string $hashed_validator, string $expiry): bool{
    ...
    public static function find_admin_token_by_selector(string $selector){
    ...
    public static function delete_admin_token(int $admin_id): bool{
    ...
    public static function find_admin_by_token(string $token){
    ...
    public static function token_validity(string $token): bool { // parse the token to get the selector and validator [$selector, $validator] = parse_token($token);
        $tokens = self::find_admin_token_by_selector($selector);
        if (!$tokens) {
            return false;
        }
    ...
    public static function remember_admin(int $admin_id, int $day = 30){
        [$selector, $validator, $token] = generate_tokens();
    
        // remove all existing token associated with the admin id
        self::delete_admin_token($admin_id);
        ...
        if (self::insert_admin_token($admin_id, $selector, $hash_validator, $expiry)) {
